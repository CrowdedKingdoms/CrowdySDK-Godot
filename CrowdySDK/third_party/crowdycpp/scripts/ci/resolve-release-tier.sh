...
# a deploy tool's "refuses an older version" guard; `v1.2.3+test` is illegal in a Docker
...
# below the bare version in a deploy tool's rollback guard, which is the trap the tag
